	...
	void down(int index) {
		if (records != 0) {
			...
			while (((2 * index) <= records || (2 * index + 1) <= records) && !flag) {
				if ((ar + (index - 1))->key < (ar + (2 * index - 1))->key && (ar + (index - 1))->key < (ar + (2 * index))->key) {
				...
				else if ((ar + (index - 1))->key < (ar + (2 * index - 1))->key) {
					Data buffer = *(ar + (index - 1));
					*(ar + (index - 1)) = *(ar + (2 * index - 1));
					*(ar + (2 * index - 1)) = buffer;
					index = (2 * index);
				}
				else if ((ar + (index - 1))->key < (ar + (2 * index))->key) {
					Data buffer = *(ar + (index - 1));
					*(ar + (index - 1)) = *(ar + (2 * index));
					*(ar + (2 * index)) = buffer;
					index = (2 * index + 1);
